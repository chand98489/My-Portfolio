

export const Bio = {
  name: "Chand Basha Panyam",
  roles: [
    "Full Stack Developer",
    
    // "Programmer",
  ],
  description:
  "As a dedicated MERN stack developer, I thrive on tackling new challenges and continuously expanding my skill set. My passion for learning drives me to stay updated with the latest advancements in web development, ensuring that I deliver high-quality, efficient, and scalable solutions. ",
  github: "https://github.com/chand98489",
  resume:"https://drive.google.com/file/d/1gH_xrlK7MS7A81pBcZlI8f4aSjL8tZ-k/view?usp=sharing",
  linkedin: "https://www.linkedin.com/in/chand-basha-panyam-79a87a269/",
  insta: "https://www.instagram.com/chand.24x/",
  Mail:"mailto:chandpanyam54@gmail.com",

};

export const skills = [
  {
    title: "Frontend",
    skills: [
      
      {
        name: "HTML",
        image: "https://www.w3.org/html/logo/badge/html5-badge-h-solo.png",
      },
      {
        name: "CSS",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/d/d5/CSS3_logo_and_wordmark.svg/1452px-CSS3_logo_and_wordmark.svg.png",
      },
      {
        name: "Tailwind CSS",
        image:
"https://tse2.mm.bing.net/th?id=OIP.Y-BbFmqRsIQWzX5BWC4mNQAAAA&pid=Api&P=0&h=180",      },

      {
        name: "JavaScript",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6a/JavaScript-logo.png/800px-JavaScript-logo.png",
      },
      {
        name: "Redux",
        image:
"https://tse4.mm.bing.net/th?id=OIP.yHO9EA1tfmLQoatRkVA4_AHaFj&pid=Api&P=0&h=180"      },
      {
        name: "Bootstrap",
        image:
          "https://getbootstrap.com/docs/5.3/assets/brand/bootstrap-logo-shadow.png",
      },
      {
        name: "React.Js",
        image:
          "data:image/svg+xml;base64,",
      },
     
    ],
  },
  {
    title: "Backend",
    skills: [
      {
        name: "Node Js",
        image: "https://nodejs.org/static/images/logo.svg",
      },
      {
        name: "Express Js",
        image:
          "data:image/png;base64,",
      },
     
     
      {
        name: "MongoDB",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/mongodb/mongodb-original-wordmark.svg",
      },
      {
        name: "Postman",
        image:
          "https://tse4.mm.bing.net/th?id=OIP.4nQ_eYPvB34aXGsenmzxFAAAAA&pid=Api&P=0&h=180",
      },

      {
        name: "MySQL",
        image:
"https://tse4.mm.bing.net/th?id=OIP.urLHYMYPFxkcs6AC4Io9vwHaHa&pid=Api&P=0&h=180"      },
      {
        name: "Rest API",
        image:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTmJoxiAXVIxedd5WnxL3yepJpACK2lmCSl9w&s",
      },
      
      
    ],
  },
  
 
  {
    title: "Others",
    skills: 
    [
      {
        name: "Java",
        image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRQfPubhH7QOXhY-TgYSBOTuvlT_ev-9SOJ3w&s",
      },
      {
        name: "Data Structures & Algorthims",
        image:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRj-l3xqeigzbmXAlyBRdkBBaFzcuLQgi1BoA&s",
      },
     
      {
        name: "SQL",
        image:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQt9ziR3N0W-lOmTIw5pU3G45b8duc-VJDdj4nGmWLCNCwryiT3COv9BeYVgPgVRD_ny_M&usqp=CAU",
      },
      {
        name: "Git",
        image:
          "https://e7.pngegg.com/pngimages/713/558/png-clipart-computer-icons-pro-git-github-logo-text-logo-thumbnail.png",
      },
      {
        name: "Computer Networks",
        image:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSqjcAZwrmOHaurdUAV3L-ZgSiAh5CIP8PBxg&s",
      },
      {
        name: "Operating System",
        image:
         "https://cdn-icons-png.flaticon.com/512/2172/2172894.png",
      },
      {
        name: "DBMS",
        image:"https://png.pngtree.com/png-vector/20220519/ourmid/pngtree-database-administrator-dbms-software-data-center-admin-secure-system-vector-png-image_46356680.jpg",
      },
     
    
      
    ],
  },
];

export const experiences = [
  {
    id: 0,
    img: "https://media.glassdoor.com/l/79/43/6b/4c/maang-technologies.jpg",
    role: "Fullstack Engineering Intern",
    company: "Maang Technologies Pvt. Ltd.",
    date: "May 2024 -August 2024",
    desc: "Working as remote developer.",
    skills: [
      "Java",
      "React",
      "JavaScript",
      "Express.js",
      "Node Js",
      "MongoDB",  
    ],
    // doc: "https://media.licdn.com/dms/image/D4D2DAQFlp60ZqHuaFQ/profile-treasury-image-shrink_1280_1280/0/1691180828512?e=1692381600&v=beta&t=mM5Y_NE5EPlQhez5FAN6NLVSKcO_Ojt_9Gq3mnFGkAQ",
  },
  {
    id: 1,
    img: "https://evodigit.com/lovable-uploads/c635732e-aeee-426e-b673-44c2c87c472d.png",
    role: "Freelance Web Developer",
    company: "EvoDigit Technolgies",
    date: "Jan 2025",
    desc: "Worked on a startup project for a couple of weeks, successfully deploying the application on Vercel and configuring a custom domain for a professional online presence.",
    skills: [
      "React",
      "TypeScript",
      "Tailwind.css",
      "Supabase",  
    ],
    // doc: "https://media.licdn.com/dms/image/D4D2DAQFlp60ZqHuaFQ/profile-treasury-image-shrink_1280_1280/0/1691180828512?e=1692381600&v=beta&t=mM5Y_NE5EPlQhez5FAN6NLVSKcO_Ojt_9Gq3mnFGkAQ",
  }


];

export const education = [
  {
    id: 0,
    img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSPbOQPosTDvQfmxRcPa2U5CEDmQWOaL9zY-A&s",
    school: "Ragiv Gandhi University of Knowledge Technologies IIIT RK Valley, KADAPA",
    date: "Oct 2021 - MAY 2025",
    grade: "8.1 CGPA",
    desc: "I am currently pursuing a Bachelor's degree Final year in CSE at RK Valley , Kadapa. I have completed 6 semesters and have a CGPA of 8.1. I have taken courses in Data Structures, Algorithms, Object-Oriented Programming, DBMS, Operating Systems, and Computer Networks, among others.",
    degree: "Bachelor of Technology - BTech, Computer Science and Engineering",
  },
  {
    id: 1,
    img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSPbOQPosTDvQfmxRcPa2U5CEDmQWOaL9zY-A&s",
    school: "Ragiv Gandhi University of Knowledge Technologies IIIT RK Valley, KADAPA",
    date: "sep 2019 - Aug 2021",
    grade: "9.4 CGPA",
    desc: "I completed my class 12th (MPC) education at RGUKT Rk Valley, Kadapa.Dankuni, where I studied Science with Computer Science.",
    degree: "PRE UNIVERSITY COURSE (PUC)",
  },
  {
    id: 2,
    img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSMuCTap3HouYp7g2HGFmAD1yl4Eb3_8CVmXgiYdUTToTo61jyygGIF4B0mRTMU_dhlxB0&usqp=CAU",
    school: "Zilla Parishad High School Alvakonda (V), Sanjamala(M) , Nandyal (Dist), AP",
    date: "Apr 2019",
    grade: "10.0 CGPA",
    desc: "I completed my class 10 education at Z P High School, ALVAKONDA ,Nandyal.",
    degree: "SSC",
  },
];

export const projects = [
  {
    id: 9,
    title: "Hungry-Hub Mern Project",
    // date: "Jun 2023 - Jul 2023",
    description:
    "Hungry Hub is an online food delivery website developed with the MERN Stack.Features seamless food browsing and secure order placement with Dummy Braintree Payment integration.Admins can manage content, update items, organize categories, and set the status of user orders.Delivers seamless food ordering experience for both users and administrators.",
    image:
      "https://i.ytimg.com/vi/xR4wZPbYheA/maxresdefault.jpg",
    tags: [

      "React.js",
      "Node.js",
      "MONGO DB",
      "Express.js",
      "restAPI",
     
     
    ],
    category: "android app",
    github: "https://github.com/chandbasha24/Hungry-Hub-Mern-Stack",
    webapp: "https://hungry-hub-mern-stack.onrender.com/",
  },

  {
    id: 10,
    title: "EvoDigit Project",
    // date: "Jun 2023 - Jul 2023",
    description:
    "Developed a web application using React, TailwindCSS, Supabase, and Framer Motion to enhance UI/UX and functionality.",
    image:
      "https://evodigit.com/lovable-uploads/c635732e-aeee-426e-b673-44c2c87c472d.png",
    tags: [
      "React",
      "Tailwind.css",
      "Supabase",  
     
    ],
    category: "android app",
    github: "https://github.com/chandbasha24/EvoDigit-Glowfolio",
    webapp: "https://chand-basha24-evo-digit-glowfolio-project.vercel.app/",
  },

  {
    id: 14,
    title: "Social Media Task Mern",
    // date: "Jun 2023 - Jul 2023",
    description:
    "In the Project User can upload the username, social media handle acc, profile pic & the admin can view the all users details by using of admin credentials.",
    image:
      "https://img.oercommons.org/780x780/oercommons/media/courseware/lesson/image/A_hrAZgVD.jpeg",
    tags: [
      "React.js",
      "Node.js",
      "MONGO DB",
      "Express.js",
     
    ],
    category: "android app",
    github: "https://github.com/chandbasha24/social-media-task_mern",
    webapp: "https://drive.google.com/file/d/1Aaf53EIYR9J8f_jZYMcJQyYK3YWABqLC/view",
  },
  // {
  //   id: 0,
  //   title: "Dice Game ",
  //   // date: "Apr 2023 - May 2023",
  //   description:
  //     "dice game , guessing of number...",
  //   image:'https://media.istockphoto.com/id/183819034/photo/two-dices.jpg?s=612x612&w=0&k=20&c=sawA-Q6tAlmzm37EiFViPCr9AT3f-PnwCM8qXrQzR2w=',
  //   tags: ["React Js", "Styled Components"],
  //   category: "web app",
  //   github: "https://github.com/chandbasha24/Dice-Game-React",
  //   webapp: "https://dice-game-react-gamma.vercel.app/",
   
  // },
  {
    id: 1,
    title: "Weather App ",
    
    description: " created the weather website using the react .",
    image:
      "https://beebom.com/wp-content/uploads/2021/05/1.-Weather-default-Windows1.1.jpg",
          tags: [
      "React Js",
         "Weather API",
    ],
    category: "web app",
    github: "https://github.com/chandbasha24/Weather-App-React",
    webapp: "https://weather-app-react-one-psi.vercel.app/",
  },
  // {
  //   id: 10,
  //   title: "Lunacal Assignment React + Tailwind  ",
    
  //   description: " created the Web page  .",
  //   image: "https://i.ytimg.com/vi/XtNkcHyVQOY/maxresdefault.jpg",
  //         tags: [
  //     "React Js",
  //        "Tailwind.Css",
  //   ],
  //   category: "web app",
  //   github: "https://github.com/chandbasha24/lunacal-assignment",
  //   webapp: "https://chandbasha24.github.io/lunacal-assignment/",
  // },

  {
    id: 20,
    title: "Relume Website UI",
    
    description: "I recreated the first screen of Relume's website in my own style. https://relume.io/",
    image: "https://tse4.mm.bing.net/th?id=OIP.-3J5VW06uZF1oVnvEZkfFAHaFj&pid=Api&P=0&h=180",
          tags: [
      "React.js",
         "Tailwind.CSS",
         "Framer Motion",
    ],
    category: "web app",
    github: "https://github.com/chandbasha24/CodeDale-Task",
    webapp: "https://chand-basha24-code-dale-task.vercel.app/",
  },

  // {
  //   id: 11,
  //   title: "Celebrare Frontend Assignment  ",
    
  //   description: " created the Web page using HTML , CSS, Js .",
  //   image: "https://cdn.dribbble.com/userupload/6957683/file/original-160f04571e69088762be22263fff1cad.png?resize=1200x900",
  //         tags: [
  //     "HTML",
  //        "CSS",
  //        "javaScript",
  //   ],
  //   category: "web app",
  //   github: "https://github.com/chandbasha24/Celebrare_Assignment-Intern/",
  //   webapp: "https://chandbasha24.github.io/Celebrare_Assignment-Intern/",
  // },

  {
    id: 2,
    title: " Movie Web clone",
    description:
" developed the Basic UI of movie website using the HTML,CSS,JavaScript",   
     image:
      "https://github.com/chandbasha24/Responsive_Movie_Web_clone/blob/main/project_home.png?raw=true",
    tags: ["HTML", "CSS", "JavaScript"],
    category: "web app",
    github: "https://github.com/chandbasha24/Responsive_Movie_Web_clone",
    // webapp: "https://brain-tumor.netlify.app/",
    
  },
  {
    id: 3,
    title: "Blog APP",
    // date: "Dec 2021 - Apr 2022",
    description:
     "created the blog website using the Mern Stack",
    image:
      "https://www.hubspot.com/hs-fs/hubfs/assets/hubspot.com/web-team/WBZ/Feature%20Pages/free-blog-maker/free-blog-maker-hero-en.webp?width=1030&name=free-blog-maker-hero-en.webp",
    tags: [ "React.js",
      "Node.js",
      "MONGO DB",
      "Express.js",
    ],
    category:"android app",
    github: "https://github.com/chandbasha24/BlogApp-Mern-Project",
    webapp: "https://github.com/chandbasha24/BlogApp-Mern-Project",
  },
  {
    id: 7,
    title: "A Scroll-Triggered Animation with ChatGPT",
    description:
          "developed my own style of motion art effects utilizing GSAP (GreenSock Animation Platform) & ScrollTrigger in React.  ",
    image:
      "https://ihatetomatoes.net/wp-content/uploads/2020/07/img_react-and-scrolltrigger.png",
    tags: ["React", "GSAP", "Scroll-trigger "],
    category: "web app",
    github: "https://github.com/chandbasha24/motion-art-effect_React",
    webapp: "https://chandbasha24.github.io/motion-art-effect_React/",
   
    
    
  },
//   {
//     id: 8,
//     title: "Tic Tac Toe Game",
//     description:
// " developed the Tic-Tac Toe game using the HTML,CSS,JavaScript",   
//      image:
//       "https://github.com/chandbasha24/Tic-Tac-Toe-Game/blob/main/Screenshot.png?raw=true",
//     tags: ["HTML", "CSS", "JavaScript"],
//     category: "web app",
//     github: "https://github.com/chandbasha24/Tic-Tac-Toe-Game",
//     // webapp: "https://brain-tumor.netlify.app/",
    
//   },
//   {
//     id: 10,
//     title: "Currency Converter",
//     description:
// " developed the  Currency Converter using the HTML,CSS,JavaScript, CurrencyAPI",   
//      image:
//       "https://github.com/chandbasha24/Currency-Converter/blob/main/Screenshot.png?raw=true",
//     tags: ["HTML", "CSS", "JavaScript,currency API"],
//     category: "web app",
//     github: "https://github.com/chandbasha24/Currency-Converter",
//     // webapp: "https://brain-tumor.netlify.app/",
    
//   },
//   {
//     id: 9,
//     title: "To-do App Assignment",
//     // date: "Jun 2023 - Jul 2023",
//     description:"The app allows users to create, update, and delete tasks, providing a simple, user-friendly interface for task management. The frontend is built using React.js, enabling dynamic rendering of the UI for adding and viewing tasks. It connects with the backend via Axios or Fetch API to handle API requests.",
//     image:
// "https://repository-images.githubusercontent.com/315986811/d74b3080-2f6a-11eb-960e-c405ce8cfc43",    tags: [
//       "React.js",
//       "Node.js",
//       "MONGO DB",
//       "Express.js",
//       "restAPI",
     
//     ],
//     category: "android app",
//     github: "https://github.com/chandbasha24/Todo-app-Assignment",
//     webapp: "https://drive.google.com/file/d/1VBFl63JJNjw6IbYDjZCDyx-MfpVkN97r/view",
//   },
 
 

 
  
 
];

export const TimeLineData = [
  { year: 2021, text: "Started my journey" },
  { year: 2022, text: "Worked as a freelance developer" },
  { year: 2023, text: "Founded JavaScript Mastery" },
  { year: 2024, text: "Shared my projects with the world" },
  { year: 2025, text: "Started my own platform" },
];
